...
int main() {
	...
	while (T--) {
		...
		map<string, int> bill, state; // state가 -1이면 INCONSISTENT, 0이면 p, 1이면 a, 2면 r
		...
		for (int i = 0; i < M; i++) {
			...
			if (akind == "p") {
				...
				state[aname] = 1;
				bill[aname] += ceil(get<0>(car[rent_car[aname]]) * stod(some) / 100); // 원가 * 파손율;
			}
			else if (akind == "r") {
				if (state[aname] == 2) {
					state[aname] = -1;
				...
				state[aname] = 2;
				bill[aname] += get<2>(car[rent_car[aname]]) * stoi(some);
			}
		}
